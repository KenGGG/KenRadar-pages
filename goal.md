...
- focused、API 契约、仪表盘一致性、来源洞察和 SSE 测试合计：132 passed。
...
- SSE 35 秒实测收到 connected 和两次 heartbeat；断开后 Web 仍健康，连接清理已有单元测试覆盖，后端重启恢复仍需后续长时间观察。
- 工作区存在较多尚未提交的 Web 代码和发布内容产物，必须先分离范围后再继续扩展。

### 4.2 阶段完成度基线

| 阶段 | 状态 | 估计完成度 | 主要结论 |
|---|---|---:|---|
| M0 后端收口 | 待验收 | 约 90% | router、responses、helpers、routes、迁移和契约测试已存在，仍需生产回归和提交收口 |
| M1 日常工作台 | 待验收 | 约 95% | 组合筛选、cursor hook、页面和真实 cursor 分页已验证，仍需移动端人工验收 |
| M2 决策仪表盘 | 待验收 | 约 90% | 聚合接口、口径测试、DailyTrend 与 PipelineStatus 已完成，需记录性能并人工对账 |
| M3 局部实时化 | 待验收 | 约 85% | SSE、EventSource、心跳和轮询降级已接线并实测，仍需后端重启恢复观察 |
| M4 来源洞察 MVP | 待验收 | 约 95% | 来源回填、日/主题指标迁移、聚合 CLI、API、hooks 和页面已完成，7 天真实回填已对账 |
...
- [ ] Web 代码与自动生成内容的变更范围已分离。
- [ ] 核心接口契约测试建立并通过。
- [ ] 前端无需修改即可调用拆分后的原接口。
- [ ] HTTP 状态码、错误格式、CORS 和认证行为不变。
- [ ] route 不再反向依赖 `app.py` 的业务函数。
- [ ] compileall、focused tests、前端 lint/build 通过。
- [ ] 重启后 live、ready、bootstrap、status 和内容详情接口正常。
...
- [ ] 单项及组合筛选均针对完整服务端数据集生效。
- [ ] cursor 翻页无重复、无明显漏项。
- [ ] 高价值阈值、今日边界和审阅口径有自动化测试。
- [ ] 空数据、慢请求、单接口失败不会导致白屏。
- [ ] 桌面端可用，移动端至少可完成筛选、打开详情和重试。
- [ ] 浅色/深色模式下成功、警告、失败、处理中状态可区分。
- [ ] 旧 `/api/awemes` 保留兼容，现有页面在迁移期间不受影响。
...
- [ ] 每个指标有 SQL/service 单元测试及人工抽样对账。
- [ ] 图表数字与筛选后的明细数量一致。
- [ ] 图表为零、仅一个点或部分数据缺失时可正常展示。
- [ ] 点击指标或图表可以进入对应明细。
- [ ] 7/30 天查询达到性能目标；90 天有结果且体验可接受。
- [ ] 仪表盘接口失败不影响内容详情、处理中心和来源管理。
...
- [ ] 页面刷新、断网恢复和后端重启后能恢复更新。
- [ ] 关闭页面后服务端连接被清理。
- [ ] 慢客户端不会造成广播线程或队列无限增长。
- [ ] SSE 失败后轮询可继续更新，恢复后不会双重请求。
- [ ] 丢失事件后完整同步能够修正前端状态。
...
- [ ] P0-7：整理当前未提交代码与 `docs/` 运行产物，完成独立、可审查的代码提交。
- [ ] P1-1：完成剩余真实验收后更新本页状态，将 M0–M4 中满足完成定义的阶段标为已完成。
